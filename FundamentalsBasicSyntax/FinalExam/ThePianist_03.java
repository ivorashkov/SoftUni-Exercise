...
public class ThePianist_03 {
    ...
    public static void main(String[] args) {
        ...
        piecesInformation.forEach((key, value) -> System.out.printf("%s -> Composer: %s, Key: %s%n", value.getPieceName(), value.getComposer(), value.getKey()));

        for (Map.Entry<String, Piece> entry : piecesInformation.entrySet()) {
            Piece currentPiece = entry.getValue();
            System.out.printf("%s -> Composer: %s, Key: %s%n", currentPiece.getPieceName(), currentPiece.getComposer(), currentPiece.getKey());
        }
